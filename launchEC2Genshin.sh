...
do 
    ...
    echo "Allocating EIP:"
    eipAllocationId=$(aws ec2 allocate-address --query '[AllocationId]' --output text)
    echo "Associating EIP with the EC2 instance"
    aws ec2 associate-address --instance-id $instanceId --allocation-id $eipAllocationId
    echo "The launched Instance Id of $instanceName is : $instanceId, with EIP allocated ID $eipAllocationId"
done
